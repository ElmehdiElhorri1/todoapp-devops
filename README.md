...
Testé en conditions réelles (pas seulement des fichiers écrits) :

- ✅ Application testée en local (SQL Server Express) et déployée sur Kubernetes (Minikube)
- ✅ Pipeline Jenkins exécuté de bout en bout : tests (3/3) → build Docker → push DockerHub → déploiement `kubectl` automatique
- ✅ Déploiement Kubernetes validé : pods `todoapp` et `mssql` en état `Running`, application accessible et fonctionnelle
- ✅ Workflow Git complet : branches `main`/`dev`, pull request créée et mergée
